/* ═══════════════════════════════════════════════════════════
   charts.js — 手写 SVG 图表引擎（零依赖）
   渲染器：lines / bars / shareBars / shareArea / shareArea2 /
   scatter / timeline / wall / lifelines / matrix / tree /
   capexGauge / gauge / triggers
   ═══════════════════════════════════════════════════════════ */
(function(){
"use strict";
...
if(!document.getElementById("cc-chart-style")){
  ...
  st.textContent=".drawline{stroke-dasharray:1;stroke-dashoffset:1;animation:ccdraw 1.4s ease forwards}.drawline.slow{animation-duration:2.2s}@keyframes ccdraw{to{stroke-dashoffset:0}}.charttext{font-family:var(--sans)}";
...
function div(cls,parent,html){ const d=document.createElement("div"); if(cls)d.className=cls; if(html!=null)d.innerHTML=html; if(parent)parent.appendChild(d); return d; }
...
function annoText(svg,x,y,text,color,size,weight,anchor){
  ...
  const t=mk("text",{x:x,y:y,"font-size":size||11.5,fill:color||"#57503D","text-anchor":anchor||"start",class:"anno"},svg);
  if(weight) t.setAttribute("font-weight",weight);
  lines.forEach((ln,i)=>{ mk("tspan",{x:x,dy:i===0?0:(size||11.5)*1.45},t).textContent=ln; });
  return t;
}
...
function legendHTML(spec,onClick){
  const wrap=div("legend");
  (spec.series||[]).forEach(s=>{
    const lg=div("lg"+(s._off?" off":""),wrap);
    lg.innerHTML='<span class="sw'+(s.dot?" dot":"")+'" style="background:'+s.color+'"></span>'+esc(s.name);
    lg.onclick=function(){ s._off=!s._off; lg.classList.toggle("off",!!s._off); onClick&&onClick(); };
  });
  return wrap;
}
...
function renderLines(el,spec){
  const W=920,H=430,m={l:66,r:spec.right?(spec.hasRight||spec.series.some(s=>s.rightAxis)?66:24):24,t:30,b:42};
  ...
  /* 网格与轴 */
  const yticks=isLog?logTicks(yd[0],yd[1]):niceTicks(yd[0],yd[1],5);
  yticks.forEach(v=>{
  ...
  const xticks=niceTicks(xl[0],xl[1],7).filter(v=>v>=xl[0]&&v<=xl[1]);
  xticks.forEach(v=>{
    mk("line",{x1:X(v),x2:X(v),y1:m.t,y2:m.t+ph,class:"grid-line","stroke-dasharray":"1 4"},svg);
    mk("text",{x:X(v),y:m.t+ph+20,"text-anchor":"middle",class:"tick-lab"},svg).textContent=Math.round(v);
  });
  mk("line",{x1:m.l,x2:m.l+pw,y1:m.t+ph,y2:m.t+ph,class:"axis-line"},svg);
  annoText(svg,m.l,m.t-14,"单位："+(spec.unit||""),"#8C8370",10.5);
  /* 右轴 */
  let YR=null;
  if(spec.right){
    const rd=spec.right.ydomain;
    YR=function(v){ return m.t+ph-(v-rd[0])/(rd[1]-rd[0])*ph; };
    niceTicks(rd[0],rd[1],4).forEach(v=>{
      mk("text",{x:m.l+pw+8,y:YR(v)+4,class:"tick-lab"},svg).textContent=(spec.right.fmt?spec.right.fmt(v):fmtN(v));
    });
    mk("line",{x1:m.l+pw,x2:m.l+pw,y1:m.t,y2:m.t+ph,class:"axis-line"},svg);
    annoText(svg,m.l+pw,m.t-14,spec.right.label||"","#1D5FBF",10.5,"700","end");
  }
  /* 线 */
  spec.series.forEach(s=>{
    if(s._off) return;
    ...
    mk("circle",{cx:X(last[0]),cy:(s.rightAxis?YR(last[1]):Y(last[1])),r:3.4,fill:s.color},svg);
  });
  /* 注释 */
  (spec.annotations||[]).forEach(a=>{
    const ax=X(a.x), ay=Y(a.y);
    annoText(svg,ax+8,ay,a.text,"#57503D",11.5,"700");
    mk("line",{x1:ax,x2:ax+6,y1:ay,y2:ay-4,class:"anno-line"},svg);
  });
  /* 交互十字线 */
...
function renderBars(el,spec){
  const W=920,H=430,cats=spec.xCats,m={l:66,r:spec.line?70:30,t:34,b:44};
  const pw=W-m.l-m.r, ph=H-m.t-m.b;
  const svg=frame(el,W,H);
  ...
  niceTicks(yd[0],yd[1],5).forEach(v=>{
    mk("line",{x1:m.l,x2:m.l+pw,y1:Y(v),y2:Y(v),class:"grid-line"},svg);
    mk("text",{x:m.l-8,y:Y(v)+4,"text-anchor":"end",class:"tick-lab"},svg).textContent=fmtN(v);
  });
  cats.forEach((c,i)=>{
    mk("text",{x:m.l+cw*(i+0.5),y:m.t+ph+20,"text-anchor":"middle",class:"tick-lab"},svg).textContent=c;
  });
  mk("line",{x1:m.l,x2:m.l+pw,y1:m.t+ph,y2:m.t+ph,class:"axis-line"},svg);
  annoText(svg,m.l,m.t-16,"单位："+(spec.unit||""),"#8C8370",10.5);
  const bw=spec.grouped? cw/Math.max(1,spec.series.filter(s=>!s._off).length)*0.62 : Math.min(64,cw*0.5);
  cats.forEach((c,i)=>{
    let acc=0, gi=0;
    spec.series.forEach(s=>{
      if(s._off) return;
      const v=s.vals[i];
      let x0;
      if(spec.grouped){ x0=m.l+cw*i+cw/2-bw*spec.series.filter(s2=>!s2._off).length/2+bw*gi; gi++; }
      else x0=m.l+cw*i+cw/2-bw/2;
      const h=(v-yd[0])/(yd[1]-yd[0])*ph;
      const y=m.t+ph-h-(spec.stacked?((acc)/(yd[1]-yd[0]))*ph:0);
      const rect=mk("rect",{x:x0,y:y,width:bw,height:Math.max(0,h),fill:s.color,opacity:spec.stacked?0.92:0.88,rx:1.5},svg);
  ...
  if(spec.line){
    ...
    niceTicks(ld[0],ld[1],4).forEach(v=>{
      mk("text",{x:m.l+pw+8,y:YR(v)+4,class:"tick-lab"},svg).textContent=fmtN(v)+"%";
    });
    mk("line",{x1:m.l+pw,x2:m.l+pw,y1:m.t,y2:m.t+ph,class:"axis-line"},svg);
    const pts=spec.line.vals.map((v,i)=>[m.l+cw*(i+0.5),YR(v)]).filter((p,i)=>spec.line.vals[i]!=null);
    drawLinePath(svg,pts,spec.line.color,2.4,null);
    pts.forEach(p=>mk("circle",{cx:p[0],cy:p[1],r:3,fill:spec.line.color},svg));
    annoText(svg,m.l+pw,m.t-16,spec.line.name,"#C2451E",10.5,"700","end");
  }
  if(spec.ksY!=null&&YR){
    const ky=YR(spec.ksY);
    mk("line",{x1:m.l,x2:m.l+pw,y1:ky,y2:ky,class:"ks-line"},svg);
    annoText(svg,m.l+pw-4,ky-6,"Kill Switch +14%","#C2451E",11,"700","end");
    mk("rect",{x:m.l,y:ky,width:pw,height:m.t+ph-ky,fill:"#C2451E",opacity:0.05},svg);
  }
  (spec.annotations||[]).forEach(a=>{
    const ax=m.l+cw*(a.xi+0.5);
    annoText(svg,ax,spec.line?m.t+34:m.t+18,a.text,"#57503D",11.5,"700","middle");
  });
  return svg;
}
...
function renderShareBars(el,spec){
  const W=920,H=430,cats=spec.xCats,m={l:56,r:30,t:34,b:44};
  const pw=W-m.l-m.r, ph=H-m.t-m.b;
  const svg=frame(el,W,H);
  const Y=function(v){ return m.t+ph-v/100*ph; };
  [0,25,50,75,100].forEach(v=>{
    mk("line",{x1:m.l,x2:m.l+pw,y1:Y(v),y2:Y(v),class:"grid-line"},svg);
    mk("text",{x:m.l-8,y:Y(v)+4,"text-anchor":"end",class:"tick-lab"},svg).textContent=v+"%";
  });
  const cw=pw/cats.length, bw=Math.min(120,cw*0.42);
  cats.forEach((c,i)=>{
    ...
    spec.series.forEach(s=>{
      ...
      if(v>=7) mk("text",{x:bx+bw/2,y:y+h/2+4,"text-anchor":"middle","font-size":10.5,fill:"#FFFDF6","font-weight":700},svg).textContent=v+"%";
    ...
    mk("text",{x:bx+bw/2,y:m.t+ph+20,"text-anchor":"middle",class:"tick-lab"},svg).textContent=c;
  });
  (spec.annotations||[]).forEach(a=>{
    const bx=m.l+cw*(a.xi+0.5);
    annoText(svg,bx,m.t+ph+38,a.text,"#57503D",11.5,"700","middle");
  });
  mk("line",{x1:m.l,x2:m.l+pw,y1:m.t+ph,y2:m.t+ph,class:"axis-line"},svg);
  annoText(svg,m.l,m.t-16,"单位："+spec.unit,"#8C8370",10.5);
  return svg;
...
function renderShareArea(el,spec){
  const W=920,H=430,m={l:56,r:24,t:30,b:42};
  const pw=W-m.l-m.r, ph=H-m.t-m.b;
  const svg=frame(el,W,H);
  const yd=spec.ydomain||[0,100];
  const Y=function(v){ return m.t+ph-(v-yd[0])/(yd[1]-yd[0])*ph; };
  ...
  yrs.filter((_,i)=>i%3===0||i===yrs.length-1).forEach(v=>{
    mk("text",{x:X(v),y:m.t+ph+20,"text-anchor":"middle",class:"tick-lab"},svg).textContent=v;
  });
...
function renderShareArea2(el,spec){
  const W=920,H=430,m={l:56,r:70,t:32,b:42};
  const pw=W-m.l-m.r, ph=H-m.t-m.b;
  const svg=frame(el,W,H);
  const yrs=spec.xYears, xl=[yrs[0],yrs[yrs.length-1]];
  const X=function(v){ return m.l+(v-xl[0])/(xl[1]-xl[0])*pw; };
  ...
  mk("line",{x1:m.l+pw,x2:m.l+pw,y1:m.t,y2:m.t+ph,class:"axis-line"},svg);
  annoText(svg,m.l+pw,m.t-14,spec.right.label,"#C2451E",10.5,"700","end");
  const lp=spec.lineRight.pts.map(p=>[X(p[0]),YR(p[1])]);
  drawLinePath(svg,lp,spec.lineRight.color,2.6,null);
  lp.forEach(p=>mk("circle",{cx:p[0],cy:p[1],r:3.4,fill:spec.lineRight.color},svg));
  const lr=svg.getBoundingClientRect();
  lp.forEach((p,i)=>{
    const c=mk("circle",{cx:p[0],cy:p[1],r:12,fill:"transparent"},svg);
  ...
  yrs.forEach(v=>{ if(v%2===0||v===2026) mk("text",{x:X(v),y:m.t+ph+20,"text-anchor":"middle",class:"tick-lab"},svg).textContent=v+(v===2026?"E":""); });
  mk("line",{x1:m.l,x2:m.l+pw,y1:m.t+ph,y2:m.t+ph,class:"axis-line"},svg);
  annoText(svg,m.l,m.t-16,"单位："+spec.unit,"#8C8370",10.5);
  return svg;
}
...
function renderScatter(el,spec){
  const W=920,H=460,m={l:70,r:30,t:34,b:44};
  const pw=W-m.l-m.r, ph=H-m.t-m.b;
  const svg=frame(el,W,H);
  ...
  niceTicks(xl[0],xl[1],7).forEach(v=>{
    mk("text",{x:X(v),y:m.t+ph+20,"text-anchor":"middle",class:"tick-lab"},svg).textContent=Math.round(v);
  });
  mk("line",{x1:m.l,x2:m.l+pw,y1:m.t+ph,y2:m.t+ph,class:"axis-line"},svg);
  annoText(svg,m.l,m.t-16,"单位："+spec.unit,"#8C8370",10.5);
  if(spec.highlight){
    ...
    annoText(svg,X(spec.highlight[0]+(spec.highlight[1]-spec.highlight[0])/2),m.t+16,"并购超级周期","#A8770F",11,"700","middle");
  }
  if(spec.divline){
    mk("line",{x1:X(spec.divline.x),x2:X(spec.divline.x),y1:m.t,y2:m.t+ph,class:"anno-line"},svg);
    annoText(svg,X(spec.divline.x)+6,m.t+30,spec.divline.label,"#8C8370",11);
  }
  const maxL=Math.max.apply(null,spec.points.map(p=>p.y));
  spec.points.forEach(p=>{
    const r=spec.bubble? 6+Math.sqrt(p.y/maxL)*20 : 5.5;
    ...
    if(!spec.bubble) annoText(svg,cx+10,cy-8,p.label,"#57503D",11);
  ...
  (spec.trend||[]).forEach(t=>{
    ...
    annoText(svg,pts[0][0]+4,pts[0][1]-12,t.label,"#57503D",11.5,"700");
  });
  return svg;
}
...
function renderLifelines(el){
  const W=940,rowH=40,m={l:150,r:120,t:26,b:34};
  ...
  [1985,1995,2005,2015,2026].forEach(v=>{
    ...
    mk("text",{x:X(v),y:H-10,"text-anchor":"middle",class:"tick-lab"},svg).textContent=v;
  });
  CC.COMPANIES.forEach((c,i)=>{
    const y=m.t+i*rowH+rowH/2;
    const col=c.era&&CC.ERAS[c.era]?CC.ERAS[c.era].color:c.color;
    mk("text",{x:m.l-12,y:y-3,"text-anchor":"end","font-size":12.5,fill:"#1A1610","font-weight":700,class:"charttext"},svg).textContent=c.name;
    mk("text",{x:m.l-12,y:y+12,"text-anchor":"end","font-size":10,fill:"#8C8370",class:"charttext"},svg).textContent=c.cn+" · "+c.born+(c.died?"–"+c.died:"–");
    ...
    c.events.forEach(ev=>{
      ...
      mk("circle",{cx:cx,cy:y,r:3.4,fill:"#FFFDF6",stroke:col,"stroke-width":1.6},svg);
    });
    band.style.cursor="pointer";
    ...
    mk("text",{x:x1+8,y:y+4,"font-size":9.5,fill:c.died?"#C2451E":"#0E7C66",class:"charttext","font-weight":700},svg).textContent=c.died?(c.died<=2009?"破产/并入":"并入"): "在场";
  });
  return svg;
}
function openCompanyModal(c){
  ...
  md.innerHTML='<h3>'+esc(c.name)+" <span style='font-size:15px;color:#57503D'>"+esc(c.cn)+"</span></h3>"
...
function renderTree(el,spec){
  ...
  CC.SCENARIOS.forEach((sc,i)=>{
    ...
    const w=sc.prob*1.15;
    const d="M"+(rootX-40)+" "+(rootY+38)+" C "+(rootX-40)+" 140, "+cx+" 130, "+cx+" "+(ny-52);
    mk("path",{d:d,fill:"none",stroke:sc.color,"stroke-width":Math.max(3,w),opacity:0.3,"stroke-linecap":"round"},svg);
    ...
    let dy=ny+168;
    mk("text",{x:cx-128,y:dy,"font-size":11,fill:sc.color,"font-weight":700,class:"charttext"},det).textContent="关键假设";
    dy+=16;
    sc.assume.forEach(a=>{ wrapCN(a,30).forEach((ln,k)=>{ mk("text",{x:cx-118,y:dy,"font-size":10,fill:"#57503D",class:"charttext"},det).textContent=(k===0?"· ":"  ")+ln; dy+=13.5; }); });
    dy+=8;
    mk("text",{x:cx-128,y:dy,"font-size":11,fill:sc.color,"font-weight":700,class:"charttext"},det).textContent="确认信号（证伪台可核查）";
    dy+=16;
    sc.confirm.forEach(a=>{ wrapCN(a,30).forEach((ln,k)=>{ mk("text",{x:cx-118,y:dy,"font-size":10,fill:"#57503D",class:"charttext"},det).textContent=(k===0?"√ ":"  ")+ln; dy+=13.5; }); });
    let open=i===0;
...
function renderGauge(el){
  ...
  function zone(v0,v1,color,op){
    ...
    mk("path",{d:"M"+p0x.toFixed(1)+" "+p0y.toFixed(1)+" A"+R+" "+R+" 0 "+large+" 1 "+p1x.toFixed(1)+" "+p1y.toFixed(1),fill:"none",stroke:color,"stroke-width":26,opacity:op},svg);
  }
  zone(0,14,"#C2451E",0.8); zone(14,25,"#A8770F",0.7); zone(25,70,"#0E7C66",0.75);
  [0,14,25,40,55,70].forEach(v=>{
    const px=cx+(R+22)*Math.cos(A(v)), py=cy-(R+22)*Math.sin(A(v));
    mk("text",{x:px,y:py+4,"text-anchor":"middle","font-size":10.5,fill:"#8C8370",class:"charttext"},svg).textContent=v+"%";
  });
  annoText(svg,cx+R*Math.cos(A(7)),cy-R*Math.sin(A(7))-22,"KILL ZONE","#C2451E",11,"700","middle");
  annoText(svg,cx+R*Math.cos(A(47)),cy-R*Math.sin(A(47))-24,"顺风区","#0E7C66",11,"700","middle");
  const cur=CC.KILL.current.g;
  const na=A(Math.min(cur,vmax));
  mk("line",{x1:cx,y1:cy,x2:cx+(R-32)*Math.cos(na),y2:cy-(R-32)*Math.sin(na),stroke:"#1A1610","stroke-width":4,"stroke-linecap":"round"},svg);
  mk("circle",{cx:cx,cy:cy,r:7,fill:"#1A1610"},svg);
  mk("text",{x:cx,y:cy+34,"text-anchor":"middle","font-size":26,fill:"#1A1610","font-weight":700,class:"charttext"},svg).textContent="+"+cur+"%";
  mk("text",{x:cx,y:cy+54,"text-anchor":"middle","font-size":11.5,fill:"#8C8370",class:"charttext"},svg).textContent=CC.KILL.current.y+"E · 大四家 capex 同比";
  /* 历史迷你柱 */
  const bx=430,bw2=150,bh=120,by=200;
  const hs=CC.KILL.hist;
  const gmax=70;
  annoText(svg,bx,by-28,"增速历史（2021–2026E）","#8C8370",10.5);
  hs.forEach((h,i)=>{
    const x=bx+i*(bw2/hs.length), w=bw2/hs.length-5;
    const val=h.g||0;
    ...
    mk("text",{x:x+w/2,y:(i%2?val>=0? y-16 : y+hh+22 : val>=0? y-4 : y+hh+11),"text-anchor":"middle","font-size":8.5,fill:"#57503D",class:"charttext"},svg).textContent=h.g==null?"—":(val>=0?"+":"")+Math.round(val)+"%";
    mk("text",{x:x+w/2,y:by+bh/2+(val>=0? 16+hh : -8),"text-anchor":"middle","font-size":8.5,fill:"#8C8370",class:"charttext"},svg).textContent=String(h.y).slice(2);
  });
  mk("text",{x:430,y:330,"font-size":10,fill:"#8C8370",class:"charttext"},svg).textContent="红 = 低于 +14% Kill 线";
  return svg;
}
